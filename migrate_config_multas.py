"""
Migration 006 — configuração de multas por atraso.

Uso:
    python migrate_config_multas.py            # dry-run (mostra o resumo e desfaz tudo no final)
    python migrate_config_multas.py --apply     # aplica de verdade (commit)
...
SQL_FILE = os.path.join(os.path.dirname(__file__), "migrations", "006_config_multas.sql")


def log(msg):
    print(f"[migrate_config_multas] {msg}")
...
def aplicar_ddl(cur):
    ...
    with open(SQL_FILE, "r", encoding="utf-8") as f:
        cur.execute(f.read())
    log("Estrutura aplicada (config_multas, checklists.dias_atraso_final/valor_multa_final, trigger).")
...
def definir_default_company(cur, company_id):
    cur.execute(
        sql.SQL("ALTER TABLE config_multas ALTER COLUMN {} SET DEFAULT %s").format(sql.Identifier("company_id")),
        (company_id,),
    )
    log("config_multas.company_id agora tem DEFAULT.")


def imprimir_resumo(cur):
    cur.execute("SELECT COUNT(*) AS total FROM config_multas")
    total = cur.fetchone()["total"]
    log("----- RESUMO -----")
    log(f"config_multas: {total} linha(s)")
