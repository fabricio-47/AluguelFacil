"""
Migration 011 — assinatura digital (contrato, checklist de entrega/devolução).

Uso:
    python migrate_assinaturas.py            # dry-run (mostra o resumo e desfaz tudo no final)
    python migrate_assinaturas.py --apply     # aplica de verdade (commit)
...
SQL_FILE = os.path.join(os.path.dirname(__file__), "migrations", "011_assinaturas.sql")


def log(msg):
    print(f"[migrate_assinaturas] {msg}")
...
def aplicar_ddl(cur):
    ...
    with open(SQL_FILE, "r", encoding="utf-8") as f:
        cur.execute(f.read())
    log("Estrutura aplicada (assinaturas, índices).")
...
def definir_default_company(cur, company_id):
    cur.execute("ALTER TABLE assinaturas ALTER COLUMN company_id SET DEFAULT %s", (company_id,))
    log("assinaturas.company_id agora tem DEFAULT.")


def imprimir_resumo(cur):
    cur.execute("SELECT COUNT(*) AS total FROM assinaturas")
    log("----- RESUMO -----")
    log(f"assinaturas: {cur.fetchone()['total']} linha(s)")
