...
Uso:
    python migrate_entregas_estoque.py            # dry-run (mostra o resumo e desfaz tudo no final)
    python migrate_entregas_estoque.py --apply     # aplica de verdade (commit)
...
SQL_FILE = os.path.join(os.path.dirname(__file__), "migrations", "007_entregas_estoque.sql")


def log(msg):
    print(f"[migrate_entregas_estoque] {msg}")
...
def aplicar_ddl(cur):
    ...
    with open(SQL_FILE, "r", encoding="utf-8") as f:
        cur.execute(f.read())
    log("Estrutura aplicada (entregas, movimentacoes_estoque, índices, trigger).")
...
def definir_defaults_company(cur, company_id):
    for tabela in ("entregas", "movimentacoes_estoque"):
        cur.execute(
            sql.SQL("ALTER TABLE {} ALTER COLUMN company_id SET DEFAULT %s").format(sql.Identifier(tabela)),
            (company_id,),
        )
    log("entregas.company_id e movimentacoes_estoque.company_id agora têm DEFAULT.")


def imprimir_resumo(cur):
    cur.execute("SELECT COUNT(*) AS total FROM entregas")
    total_entregas = cur.fetchone()["total"]
    cur.execute("SELECT COUNT(*) AS total FROM movimentacoes_estoque")
    total_movimentacoes = cur.fetchone()["total"]
    cur.execute("SELECT COUNT(*) AS total FROM branches")
    total_branches = cur.fetchone()["total"]
    log("----- RESUMO -----")
    log(f"entregas: {total_entregas} linha(s)")
    log(f"movimentacoes_estoque: {total_movimentacoes} linha(s)")
    log(f"branches (já existia desde a Fase 1): {total_branches} linha(s)")
